...
./Configure linux-x86_64 --prefix=$LV_LIBSSH_PATH/cross_compile/openssl-1.1.0f/build
make -j8
make -j8 install
cd $LV_LIBSSH_PATH
```
...
cmake -DOPENSSL_ROOT_DIR=$LV_LIBSSH_PATH/cross_compile/openssl-1.1.0f/build ..
make -j8
cd $LV_LIBSSH_PATH
```
...
cp -r cross_compile/openssl-1.1.0f/build/lib/libcrypto.so lib/.
cp -r cross_compile/openssl-1.1.0f/build/lib/libcrypto.so.1.1 lib/.
cp -r cross_compile/openssl-1.1.0f/build/lib/libssl.so lib/.
cp -r cross_compile/openssl-1.1.0f/build/lib/libssl.so.1.1 lib/.
...
/////////////////////
```console
g++ main.cpp -L./lib -I./include -lcrypto -lssl -lssh
```