...
int web_server2(ssh_session session)
{
	int rc;
	ssh_channel channel;
	char buffer[256];
...
int web_server3(ssh_session session)
{
	int rc;
	ssh_channel channel;
	char buffer[256];
	...
	// 	printf("b4 ssh_channel_accept_forward 1\n");
	// channel = ssh_channel_accept_forward(session, 60000, &port);
	// if (channel == NULL)
	// {
	// 	fprintf(stderr, "Error waiting for incoming connection: %s\n",
	...
	// 	return SSH_ERROR;
	// }
	// 	printf("b4 ssh_channel_accept_forward 2\n");

	printf("b4 ssh_channel_is_open  aa:%d\n", ssh_channel_is_open(channel));
	// printf("b4 ssh_channel_is_eof  aa:%d\n", ssh_channel_is_eof(channel));

	while ( false && ssh_channel_is_open(channel)) // &&
		// !ssh_channel_is_eof(channel))
	{
		struct timeval timeout;
		ssh_channel in_channels[2], out_channels[2];
		...
		FD_ZERO(&fds);
		FD_SET(0, &fds);
		printf("b4 ssh_get_fd 1\n");
		FD_SET(ssh_get_fd(session), &fds);
		printf("b4 ssh_get_fd 2\n");
		maxfd = ssh_get_fd(session) + 1;
		...
		if (out_channels[0] != NULL)
		{
			...
			if (nbytes > 0)
			{
				...
				nwritten = write(0, helloworld, nbytes);
				if (nwritten != nbytes) return SSH_ERROR;
			}
		}
...
int direct_forwarding2(ssh_session session)
{
	ssh_channel forwarding_channel;
	int rc;
	const char *http_get = "GET / HTTP/1.1\nHost: www.google.com\n\n";
	        ...
	        !ssh_channel_is_eof(forwarding_channel))
	{
		...
		printf("b4 ssh_select\n");
		ssh_select(in_channels, out_channels, maxfd, &fds, &timeout);
		printf("after ssh_select\n");
...
int direct_forwarding3(ssh_session session)
{
	ssh_channel forwarding_channel;
	int rc;
	const char *http_get = "GET / HTTP/1.1\nHost: www.google.com\n\n";
...
int web_server4(ssh_session session)
{
	int rc;
	ssh_channel channel;
	char buffer[4096];
	...
	while (1)
	{
		printf("b4 ssh_channel_accept_forward\n");
		channel = ssh_channel_accept_forward(session, 60000, &port);
		if (channel == NULL)
		{
			fprintf(stderr, "Error waiting for incoming connection: %s\n",
		...
		while (ssh_channel_is_open(channel) && !ssh_channel_is_eof(channel))
		{
			...
			// if(nbytes == 0) ssh_channel_send_eof(channel);
